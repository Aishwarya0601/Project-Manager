...
render()
{
    const {projectname, projectinfo,userinfo}=this.state
    console.log("prorender")
    return(
        ...
        <center> <button className="smallbutton" onClick= {()=>{this.setState({projectinfo:JSON.parse(UserService.getallinfoproject(JSON.parse(userinfo).projectassigned,2))});this.setState({show:true}) ;}} >Show</button></center>
